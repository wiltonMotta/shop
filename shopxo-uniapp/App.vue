    ...
    export default {
        globalData: {
            data: {
                // 基础配置
                // 数据接口请求地址
                request_url:'https://www.zhixiushop.cn/',
                ...
                // 每日打开奖励积分触发日期记录（避免同日重复请求）
                daily_open_reward_handle_last_date: '',
            ...
            /**
             * 每日打开奖励积分（每天最多一次，静默请求）
             */
            daily_open_reward_handle() {
                // 当天已处理过则本次运行期不再重复请求
                var date = this.get_date('yyyy-MM-dd');
                if (this.data.daily_open_reward_handle_last_date == date) {
                    return false;
                }
                this.data.daily_open_reward_handle_last_date = date;

                // 已登录用户才请求奖励（未登录无用户可发放）
                if (this.get_user_cache_info() == null) {
                    return false;
                }
                ...
                var self = this;
                uni.request({
                    url: self.get_request_url('Reward', 'index', 'userloginrewardintegral'),
                    method: 'POST',
                    data: {},
                    dataType: 'json',
                    success: (res) => {
                        // 实际赠送积分则轻提示、其余情况静默处理
                        if (res.data.code == 0 && (res.data.data || null) != null && (res.data.data.integral || 0) > 0) {
                            self.showToast('每日奖励积分 +' + res.data.data.integral);
                        }
                    },
                });
            },
        ...
        onShow(params) {
            ...
            // 每日打开奖励积分（每天最多一次）
            this.globalData.daily_open_reward_handle();
