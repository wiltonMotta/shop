    ...
    private function LoginGiveIntegral($params)
    {
        ...
        if($ret['code'] == 0)
        {
            ...
            if(!empty($give_integral))
            {
                ...
                if(Db::name('User')->where(['id'=>$params['user_id']])->inc('integral', $give_integral)->update() === false)
                {
                    return DataReturn('登录奖励积分失败', -10);
                }

                // 积分日志
                IntegralService::UserIntegralLogAdd($params['user_id'], $user_integral, $give_integral, '登录奖励积分', 1);
